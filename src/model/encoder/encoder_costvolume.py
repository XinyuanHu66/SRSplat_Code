...
class EncoderCostVolume(Encoder[EncoderCostVolumeCfg]):
    ...
    def forward(
        ...
        save_tensor_as_png(context["Texture"][0][0],'outputs/texmap-0')
        save_tensor_as_png(context["Texture"][0][1],'outputs/texmap-1')
        
        # pdb.set_trace()

        Texture_maps = self.keypoint_scorer(context["image"])

        save_tensor_as_png(Texture_maps[0].unsqueeze(0).repeat(3,1,1),'outputs/score_map-0')
        context["score_maps"] = rearrange(Texture_maps, "(b v) h w -> b v 1 h w",b=b,v=v).repeat(1,1,3,1,1)
        ...
        depths, densities, raw_gaussians, raw_gaussians_feats = self.depth_predictor(
            in_feats,
            context["intrinsics"],
            context["extrinsics"],
            context["near"],
