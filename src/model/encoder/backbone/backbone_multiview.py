...
from .unimatch.components.encoder import EncoderMARINER
from .unimatch.components.decoder import DecoderMARINER
from .unimatch.components.attention import PatchMatchAttentionMARINER
from .unimatch.components.archs import ArchitectureMARINER
import copy
...
class BackboneMultiview(torch.nn.Module):
    ...
    def extract_feature(self, images):
        ...
        features = self.backbone(concat,b,v,3)
    ...
    def save_tensor_images(self,tensor, output_dir, prefix='img'):
        """
        将4D图像tensor保存为单张图片
        参数:
            tensor: shape 为 [N, 3, H, W] 的torch.Tensor
            output_dir: 保存路径
            prefix: 文件名前缀
        """
        ...
        print(f"共保存 {tensor.size(0)} 张图片到 {output_dir}/")
    ...
    def forward(
        ...
        # cur_tex_list = [x[0] for x in tex_features_list]
