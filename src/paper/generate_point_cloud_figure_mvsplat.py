...
SCENES = (
    ...
    # ("3ac32347c3ff7d38", 46, 91, 6.0, [95], 1.4, 19),
    ("aadc1e2dc74fd644", 2, 52, 6.0, [95], 1.4, 19),
...
def generate_point_cloud_figure(cfg_dict):
    ...
    model_wrapper = ModelWrapper.load_from_checkpoint(
        ...
        losses=[],
        step_tracker=None,
        pointdecoder_cfg=cfg.model.decoder,
    )
    model_wrapper.eval()
    ...
    for idx, (scene, *context_indices, far, angles, line_width, cam_div) in enumerate(
        ...
        gaussians = encoder.forward(
            1, example["context"], False, visualization_dump=visualization_dump
        )
        
        output_context = decoder.forward(
            gaussians,
        ...
        selected_gradient_mask, k_num = model_wrapper.score_based_mask_calculation(example,example["context"]["score_maps"])
        # gaussians.opacities[~selected_gradient_mask] = 0.0
        # import pdb; pdb.set_trace()
        # fine_color, gaussians = model_wrapper.rendering_fine(example, gaussians, selected_gradient_mask, k_num, output_context)
        ...
        selected_gradient_mask = rearrange(selected_gradient_mask,"() (v h w spp) ... -> h w spp v ...", v=2, h=h, w=w)
        mask = mask & selected_gradient_mask

        def trim(element):
            element = rearrange(
                element, "() (v h w spp) ... -> h w spp v ...", v=2, h=h, w=w
            )
            # if(selected_gradient_mask==None):
